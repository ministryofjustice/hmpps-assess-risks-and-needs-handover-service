...
find . $EXCLUDES -prune -o -type f -exec sed -i.bak \
  -e "s/hmpps-assess-risk-and-needs-handover-service/$PROJECT_NAME/g" \
  -e "s/Hmpps Assess Risk And Needs Handover Service/$PROJECT_DESCRIPTION/g" \
  -e "s/HmppsAssessRiskAndNeedsHandoverService/$CLASS_NAME/g" \
  -e "s/hmppsassessriskandneedshandoverservice/$PACKAGE_NAME/g" {} \; -exec rm '{}.bak' \;

echo "Performing directory renames"

# move package directory to new name
BASE="kotlin/uk/gov/justice/digital/hmpps"
mv "src/test/${BASE}/hmppsassessriskandneedshandoverservice" "src/test/$BASE/$PACKAGE_NAME"
mv "src/main/${BASE}/hmppsassessriskandneedshandoverservice" "src/main/$BASE/$PACKAGE_NAME"
...
# rename kotlin files
mv "src/main/$BASE/$PACKAGE_NAME/HmppsAssessRiskAndNeedsHandoverService.kt" "src/main/$BASE/$PACKAGE_NAME/$CLASS_NAME.kt"
